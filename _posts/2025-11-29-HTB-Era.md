...
```bash
❯ nmap -p21,80 -sCV -Pn -oN targeted $target                                                                                                                                                             
...
```bash
❯ echo "$target era.htb" | sudo tee -a /etc/hosts
...
Tried logging in as anonymous but if nmap doesn’t show this part it’s unlikely it’s gonna work.
...
```bash
❯ gobuster -q dir -u http://era.htb -w /usr/share/seclists/Discovery/Web-Content/directory-list-2.3-medium.txt
...
```bash
❯ domain=era.htb; wfuzz -c -w /usr/share/seclists/Discovery/DNS/bitquark-subdomains-top100000.txt -u http://$domain -H "Host: FUZZ.$domain" --hh=154
...
```bash
❯ ffuf -u http://file.era.htb/download.php\?id\=FUZZ -w id.txt -H "Cookie: PHPSESSID=frng9vk6rfmbi0t509mplee65k" 
...
```bash
❯ sudo john -w=/usr/share/wordlists/rockyou.txt hashes.txt 
...
🧠 *Goal:* Save the original signature before modifying the binary.
...
> 
- `shell` is your modified version — possibly with injected code, a payload, etc.
